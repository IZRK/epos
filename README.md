...
The map bundle (`public/assets/js/map.js`) and copied Leaflet assets (`public/assets/vendor/leaflet/`, including its license) are generated by `scripts/build-map.mjs` during build/dev and are not tracked in Git. Edit `src/map/` instead of the bundle. GeoJSON, PMTiles, imported content and media remain versioned build inputs; normal builds do not download or regenerate them.
...
- Karst: [Gostinčar & Stepišnik, PANGAEA.965529](https://doi.org/10.1594/PANGAEA.965529), CC BY 4.0. The original Figure 6 shapefile has 12,734 polygons, of which 6,322 are classified as karst. The eight lithological classes remain in the attributes; the map uses the reference webmap's gray carbonate fill and translucent white boundaries, not a new classification palette. The broken, empty 2016 layer in the stations map is explicitly replaced by this 2023 dataset.
...
Station layers use small local GeoJSON files and ordinary Leaflet markers—no station PMTiles, range requests, tile decoding, or marker rebuilds on pan/zoom. Source symbol rules are respected: excluded categories do not acquire invented fallback dots. Repeated NFO station records across the legacy, 2024 and red-symbol override layers resolve to one marker, preferring the topmost explicitly styled copy. The PPTJ1/TTPJ rename is resolved by matching equipment and site within five metres. Disabling a layer restores the next available copy. Raw source records remain in the attribute tables. Popup titles use station codes; building/site descriptions stay in All attributes. Nearby distinct stations remain accessible with Previous/Next. There is no click highlight or selection-triggered tile redraw. Popup labels wrap at spaces, not inside attribute names; mobile popups stack labels above values.

`node scripts/publish-stations.mjs` copies the source point GeoJSON into the public map directory and removes obsolete station archives/duplicate attribute files. Reference refreshes and offline archive rebuilds also call it. PMTiles remains only for polygon and fault geometry. Browser checks compare actual marker counts with every enabled source layer before and after navigation. Run them in each engine with `BROWSER=chromium`, `BROWSER=firefox`, or `BROWSER=webkit` (install browsers with `npx playwright install --with-deps chromium firefox webkit`).

Polygon and line tiles are drawn into clipped tile canvases, preserving the MVT buffer without overlapping fills or drawing artificial cut edges. Point symbols stay keyboard-accessible Leaflet markers; polygon/line clicks use canvas hit testing. Symbols, colors, opacity, line widths and label settings come from the original webmap definitions. In the SLO KARST map, black fault lines use three activity-dependent widths. Fault names follow the lines above the original minimum label scale, with white halos; a shared viewport-wide SVG pass avoids clipping labels at tile boundaries and suppresses overlapping names. Original station picture symbols are stored locally because they are required to reproduce the reference, not unused downloads.
...
`node scripts/map-reference-style.mjs` reapplies the original display styles and labels from saved webmap definitions without downloading or rebuilding geometry. The reference-layer rebuild also calls this shared styling step. Browser regressions verify grayscale karst pixels, fault labels, label visibility after toggling/zooming, and original legend styles; label placement is a local Leaflet implementation, not ArcGIS's proprietary labeling engine.
