...
Station layers use small local GeoJSON files and ordinary Leaflet markers—no station PMTiles, range requests, tile decoding, or marker rebuilds on pan/zoom. All records in an enabled station layer are displayed at every zoom; categories omitted from the original renderer get an explicit fallback symbol and legend entry. Nearby/overlapping markers are accessible with Previous/Next in the popup. Selection is highlighted in cyan and cleared when the popup closes or its layer is disabled. Popup labels wrap at spaces, not inside attribute names; mobile popups stack labels above values.
